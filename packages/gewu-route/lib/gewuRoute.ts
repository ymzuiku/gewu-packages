...
export const gewuRoute = {
  ...
  goBack: (data?: any) => {
    if (isIOSWechat()) {
      gewuRoute.replace(historyProxy.stack[historyProxy.stack.length - 1].url, data);
    } else {
  ...
  isBack: () => {
    const lastStack = gewuRoute.getLastStack();
    if (lastStack.url === gewuRoute.nowFullUrl()) {
