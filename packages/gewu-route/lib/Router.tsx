...
import { gewuRoute } from "./gewuRoute";
...
function Item({
  ...
  return useMemo(() => {
    ...
    if (preload && !preloadCache[path]) {
      ...
      setTimeout(() => {
        preload.forEach(gewuRoute.preload);
      }, preloadDelay || 500);
    }
      ...
      <routeCtx.Provider value={{ stack, isBack: last && stack.time + 100 < Date.now(), last }}>
        <Suspense key={path} fallback={<div></div>}>
          <div
            ...
            onTouchStart={last ? void 0 : stop}
            onMouseDown={last ? void 0 : stop}
          >
            <Component />
          </div>
        </Suspense>
...
export function Router({ NotFoundComponent = Empty }: RouterProps) {
  if (!historyProxy.stack.length) {
    historyProxy.stack = [gewuRoute.newStack(gewuRoute.nowFullUrl())];
  }
